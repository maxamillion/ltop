...
func TestNetworkMetrics(t *testing.T) {
	...
	if iface.BytesRecv < 0 || iface.BytesSent < 0 {
		t.Error("Bytes values cannot be negative")
	}
	
	if iface.PacketsRecv < 0 || iface.PacketsSent < 0 {
		t.Error("Packet values cannot be negative")
	}
	
	if iface.Speed < 0 {
		t.Error("Interface speed cannot be negative")
	}
}
...
func TestProcessMetrics(t *testing.T) {
	...
	if proc.MemoryRSS < 0 {
		t.Error("Memory RSS cannot be negative")
	}
